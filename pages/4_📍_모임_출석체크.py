...
with col_nav_left:
    if st.button("⬅️ 메인 메뉴로", key="p4_top_back_home_btn", use_container_width=True):
...
if st.button("⬅️ 메인 메뉴로 돌아가기", key="p4_bottom_back_home_btn", use_container_width=True):
    st.session_state.current_page = "home"
    try:
        st.switch_page("app.py")
    except Exception:
