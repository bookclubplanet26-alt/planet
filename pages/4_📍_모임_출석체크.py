...
from utils import (
    ...
    ATTENDANCE_WEBHOOK_URL, append_attendance_to_google_sheet_async, get_club_season_code
)
...
if not google_user:
    ...
    with col_g1:
        g_email_input = st.text_input("Google 계정 이메일 주소", placeholder="example@gmail.com", key="p4_att_google_login_email")
    with col_g2:
        st.write("")
        st.write("")
        login_submitted = st.button("🔑 Google 인증", key="p4_att_google_login_btn", type="primary", use_container_width=True)
    ...
    if st.button("🚪 다른 이메일로 인증 (로그아웃)", key="p4_att_google_logout_btn"):
        st.session_state.google_user = None
        st.rerun()
    ...
    if not meetings:
        ...
        if not my_meetings:
            ...
            selected_meeting_label = st.selectbox("출석체크할 모임 선택", list(meeting_dict.keys()), key="p4_att_meeting_select")
            ...
            if not my_rsvp:
                ...
                if already_checked_in:
                    ...
                    if is_admin:
                        if st.button("👑 [운영진 테스트] 이 모임 출석 기록 초기화하고 구글시트 재전송 테스트하기", key="p4_admin_reset_att_btn"):
                    ...
                    att_choice = st.radio(
                        ...
                        ["📖 정규모임", "🛋️ 라운징"], 
                        horizontal=True, 
                        key="p4_att_type_radio_choice"
                    )

                    if "라운징" in att_choice:
                        att_type_name = "라운징"
                        book_read_input = st.text_input("📖 지참 책 제목 (선택)", placeholder="지참한 책이 있다면 입력해주세요 (선택)", key="p4_att_book_read_input")
                        book_review_input = st.text_area("💬 책에 대한 간단한 감상평 (선택)", placeholder="책을 읽고 느낀 점이나 공유하고 싶은 한 줄 생각을 적어주세요 (선택)", key="p4_att_book_review_input", height=80)
                    else:
                        att_type_name = "정규모임"
                        book_read_input = st.text_input("📖 지참 책 제목", placeholder="예: 데미안, 사피엔스 등", key="p4_att_book_read_input")
                        book_review_input = st.text_area("💬 책에 대한 간단한 감상평 (선택)", placeholder="책을 읽고 느낀 점이나 공유하고 싶은 한 줄 생각을 적어주세요 (선택)", key="p4_att_book_review_input", height=80)

                    # 운영진 테스트 옵션
                    bypass_time = False
                    if is_admin:
                        bypass_time = st.checkbox("👑 [운영진 테스트] 시간/위치 제한 해제", key="p4_att_admin_bypass_time")
                    ...
                    if st.button("✅ 출석체크 완료하기", type="primary", use_container_width=True, key="p4_att_confirm_btn"):
                        # 위치 검증 (기본값: 현장 도착)
